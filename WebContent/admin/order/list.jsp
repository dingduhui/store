							...
							<c:forEach items="${allOrders}" var="o" varStatus="status">
								<tr onmouseover="this.style.backgroundColor = 'white'"
									...
									<td style="CURSOR: hand; HEIGHT: 22px" align="center"
										...
										<c:if test="${o.state==2}"><a href="/store/AdminOrderServlet?method=updateOrderByOid&oid=${o.oid}">发货</a></c:if>
										<c:if test="${o.state==3}">已发货</c:if>
										<c:if test="${o.state==4}">订单完成</c:if>
									<td align="center" style="HEIGHT: 22px">
									<input type="button" value="订单详情" class="btn1"  id="${o.oid}"/>
	...
	$(".btn1").click(function(){
		...
		if(txt="订单详情"){
			...
			$.post(url,obj,function(data){
				$tb.html("");
				var th="<tr><th>商品</th><th>名称</th><th>数量</th><th>小计</th></tr>";
				$tb.append(th);
				$.each(data,function(i,obj){
					var tr="<tr><td><img src='/store/"+obj.product.pimage+"' width='50px'/></td><td>"+obj.product.pname+"</td><td>"+obj.quantity+"</td><td>"+obj.total+"</td></tr>"
					$tb.append(tr);
				})
			},"json");
			this.value="关闭";
